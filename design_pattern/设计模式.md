...
- **实现：**
  ...
  - **浅拷贝：**Object.clone()方法只是浅克隆，如果要使用Object.clone()方法，该类必须实现Cloneable接口。否则，调用clone方法会抛出CloneNotSupportedException异常，克隆对象的属性值，其中基本类型和String都是进行拷贝，String类型是常量final类，如果修改克隆后的对象，相当于创建new String("abc")，所以也进行拷贝。如果原型类包含引用对象，而克隆后对象对该引用对象进行修改，会把原型对象的引用对象的值改变，引用类型只是进行浅拷贝，克隆后的对象还是同一个内存地址引用。
